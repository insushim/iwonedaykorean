...
function buildFallbackSession(
  ...
  const buildSessionPassage = (
    passage: NonNullable<ReturnType<typeof findPassage>>,
  ): SessionPassage => {
    const questions = getQuestionsByPassageId(passage.id);
    const fallbackQuestions = SEED_QUESTIONS.filter(
      (q) => q.passageId !== "",
    ).slice(0, 2);
  ...
  if (nonfictionPassage)
    sessionPassages.push(buildSessionPassage(nonfictionPassage));
  ...
  const totalQuestions =
    sessionPassages.reduce((sum, p) => sum + p.questions.length, 0) +
...
export default function QuizSessionPage() {
  ...
  const [currentSection, setCurrentSection] =
    useState<SectionType>("nonfiction");
  ...
  const sectionQuestions = useMemo((): SessionQuestion[] => {
    ...
    const passage = session.passages.find(
      (p) => p.type === (currentSection as PassageType),
    );
  ...
  const totalAnswered = useMemo(() => {
    ...
    const grammarAnswered = session.grammarQuestions.filter(
      (q) => q.isCorrect === true,
    ).length;
  ...
  const isSectionComplete = useCallback(
    (section: SectionType): boolean => {
      ...
      if (section === "grammar")
        return session.grammarQuestions.every((q) => q.isCorrect === true);
      const passage = session.passages.find(
        (p) => p.type === (section as PassageType),
      );
    ...
    if (session) {
      ...
      if (correct) {
        ...
        if (currentQuestion.attempts === 0) {
          updatedSession.correctOnFirstTry =
            (updatedSession.correctOnFirstTry || 0) + 1;
        }
      }
          ...
          {readingMode && currentPassage ? (
            ...
            >
              <div className="p-5 lg:p-8">
                <div className="mb-4">
                  <span
                    className={`inline-block px-2.5 py-1 rounded-lg text-xs font-medium ${sectionConfig[currentSection].bgColor} ${sectionConfig[currentSection].color}`}
                  >
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    {currentSection === "grammar" && (
                      <span
                        className={`inline-block mb-2 px-2 py-0.5 rounded text-xs font-medium ${sectionConfig.grammar.bgColor} ${sectionConfig.grammar.color}`}
                      >
                        문법
                      </span>
                    )}
                    ...
                    <div className="space-y-2.5">
                      {currentQuestion.choices.map((choice: Choice) => {
                        ...
                        const isAnswer =
                          choice.number === currentQuestion.correctAnswer;
                      ...
                      {xpPopup !== null && (
                        ...
                        >
                          <div className="flex items-center gap-1.5">
                            <Sparkles className="w-4 h-4" />+{xpPopup} XP
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
